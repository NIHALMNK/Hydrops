...
/**
 * Universal Centered Stacked Cards Scrolling Engine.
 *
 * Runs seamlessly on all device viewports (Mobile, Tablet, Desktop, Ultrawide):
 * - Pins `#craft-pinned-viewport` at `top 60px` (or `top 50px` on mobile).
 * - `#craft-pinned-viewport` is a 100vh flexbox container holding every active card
 *   at the EXACT VISUAL CENTER OF THE SCREEN throughout the stacking animation.
 * - Card 1 starts centered in place.
...
export function initCraftsmanship(section: HTMLElement): () => void {
  ...
  // 1. Section Header Entrance (Scrolls into view naturally before card pinning begins)
  if (label) {
    gsap.fromTo(
      label,
      { opacity: 0, y: 20 },
  ...
  // 2. Set Initial Positions: Card 1 centered in place, Cards 2-4 below viewport center
  ...
  // 3. Master Stacked Cards Timeline (Operates across Mobile, Tablet, Desktop & Ultrawide)
